<?php
/**
* $Id: groups.sef.php 837 2010-11-17 12:03:35Z eaxs $
* @package   Projectfork
...
function PFgroupsBuildRoute(&$query)
{
    ...
    if(isset($query['id']))
    {
        ...
        if(!array_key_exists($u, $cached_u)) {
            $db = JFactory::getDBO();
            $q = "SELECT title FROM #__pf_groups WHERE id = '$u'";
                   $db->setQuery($q);
                   $uname = $db->loadResult();
...
function PFgroupsParseRoute($segments, $task = NULL)
{
    $vars  = array();
