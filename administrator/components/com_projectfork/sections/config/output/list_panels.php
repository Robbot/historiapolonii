<?php
/**
* $Id: list_panels.php 863 2011-03-21 00:00:29Z angek $
* @package    Projectfork
* @subpackage Config
...
<div class="pf_container">
    <div class="pf_header componentheading">
        <h1><?php echo PFformat::Lang('CONFIG');?> :: <?php echo PFformat::Lang('PANELS');?></h1>
    </div>
    <div class="pf_body">
                ...
                <?php
					...
					$showtips =  (int) $config->Get('tooltip_help');
 	                ...
 	                {	
 	  	                ...
 	  	                $link_edit  = PFformat::Link("section=config&task=form_edit_panel&id=$row->id".$filter);
                        $link_pub   = PFformat::Link("section=config&task=task_publish&type=panel&id=$row->id".$filter);
                        $link_unpub = PFformat::Link("section=config&task=task_unpublish&type=panel&id=$row->id".$filter);
                        ...
                        <tr class="row'.$k.' sectiontableentry'.($k + 1).'">
  	                        ...
  	                        <td class="pf_panel_title item_title"><strong>'.PFformat::Lang($row->title).'</strong>'.$is_core;
...
echo $form->HiddenField("type", 'panel');
