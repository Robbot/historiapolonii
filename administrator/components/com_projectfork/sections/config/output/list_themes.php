<?php
/**
* $Id: list_themes.php 863 2011-03-21 00:00:29Z angek $
* @package    Projectfork
* @subpackage Config
...
<div class="pf_container">
    <div class="pf_header componentheading">
        <h1><?php echo PFformat::Lang('CONFIG');?> :: <?php echo PFformat::Lang('THEMES');?></h1>
    </div>
    <div class="pf_body">
                ...
                <?php
					...
					$showtips =  (int) $config->Get('tooltip_help');
 	                ...
 	                {	
                        ...
                        $link_edit  = PFformat::Link("section=config&task=form_edit_theme&id=$row->id");

 	  	                $is_default = "<a href='".PFformat::Link("section=config&task=task_default_theme&id=$row->id")."'>"
 	  	                            . $img_default2
 	  	                            . "</a>";
 	  	                ...
 	  	                $published = "<a href='".PFformat::Link("section=config&task=task_publish&type=theme&id=$row->id")."'>"
                                   . $img_pub2
                                   . "</a>";            
 	  	 
 	  	                if($row->enabled  == '1') { 
 	  	                    $published = "<a href='".PFformat::Link("section=config&task=task_unpublish&type=theme&id=$row->id")."'>"
 	  	                               . $img_pub1
 	  	                               . "</a>";
 	  	                }
                        ...
                        <tr class="row'.$k.' sectiontableentry'.($k + 1).'">
  	                       ...
  	                       <td class="pf_theme_title item_title"><strong>'.PFformat::Lang($row->title).'</strong>'.$is_core;
  	                       
  	                    $html .= $table->Menu();
                        if($user->Access('form_edit_theme', 'config')) {
...
echo $form->HiddenField("type", 'theme');
