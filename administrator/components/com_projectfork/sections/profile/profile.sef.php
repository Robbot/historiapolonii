<?php
/**
* $Id: profile.sef.php 837 2010-11-17 12:03:35Z eaxs $
* @package   Projectfork
...
function PFprofileBuildRoute(&$query)
{
    static $cached_ws = array();
    static $cached_u  = array();
    $segments = array();
    ...
    if(isset($query['id']))
    {
        ...
        if(!array_key_exists($u, $cached_u)) {
            $db = JFactory::getDBO();
            $q = "SELECT username FROM #__users WHERE id = '$u'";
                   $db->setQuery($q);
                   $uname = $db->loadResult();
           
            $cached_u[$u] = JFilterOutput::stringURLSafe($uname);
            unset($db);
        }
        
        $segments[] = $query['id'].':'.$cached_u[$u];
        unset($query['id']);
    }
...
function PFprofileParseRoute($segments, $task = NULL)
{
    $vars  = array();
