<?php
/**
 * JFUploader 3.1 Freeware - for Joomla 1.5.x - 2.5.x
 *
...
function getEmailInputBox($param, $value, $class) {
 return '
		 <tr '. HTML_joomla_flash_uploader::getRowClass() .'>
			<td '.$class.'>'.JText::_('E_S_'.strtoupper($param)).'</td>
			<td><input onBlur="removeSpaces(this);validEmail(this, \'Error e-mail\')" type="text" class="w250" maxsize="100"
				name="'.$param.'" value="'.$value.'" /></td>
			<td>'.JText::_('E_D_'.strtoupper($param)).'</td>
		</tr>
		';
...
echo '
  ...
  <div id="flashcontent"><div class="noflash">' . JText::_('MES_NO_FLASH') . '</p></div></div>
...
echo '
   swfobject.embedSWF("'.$pathfix.'components/com_jfuploader/tfu/tfu_3.1.swf?joomla=true&lang='.$lang.'&session_id='.session_id().'&base='.$base_dir.'&relative_dir='.$relative_dir.$extra_settings.'&'.$row->swf_text.'", "flashcontent", "'.$width.'", "'.$height.'", "8.0.0", "", flashvars, params, attributes);
';
echo <<< HTML
	</script>
...
echo '<tr '. HTML_joomla_flash_uploader::getRowClass() .'>
    ...
    echo '<input type="hidden" name="use_smtp" value="false"/>';
}
...
<form action="index2.php" method="post" name="adminForm" id="adminForm">
  ...
  <p>'.JText::_('U_FILTER').' <input id="filter" name="filter" onkeyup="doFilter(this, \'usermappings\');" type="text" /> &nbsp;<a onclick="blur(); return resetUserFilter();" href="#"><img src="images/cancel.png" style="margin:-3px;" height=14 border="0" /></a></p>
