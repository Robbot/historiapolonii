...
class JElementColorpicker extends JElement
{
	...
	function fetchElement($name, $value, &$node, $control_name='params'){
		...
		$output  = "<input type=\"text\" style=\"background-color: ".$value."\" 
					name=\"".$control_name."[".$name."]\" value=\"".$value."\">"; 
		$output .= "<input type=\"button\" value=\"#\" 
					onclick=\"showColorPicker(this, document.getElementsByName('".$control_name."[".$name."]')[0])\">";
		return $output;
	}
}
