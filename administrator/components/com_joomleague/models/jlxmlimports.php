		...
		if ((isset($xmlData->record)) && (is_object($xmlData->record)))
		{
			foreach ($xmlData->record as $value)
			{
				...
				if ($xmlData->record[$i]['object']=='JoomLeague')
				{
					...
					JError::raiseNotice(0,JText::_('JL_ADMIN_XML_IMPORT_RENDERING_093'));
				}
				...
				if ($xmlData->record[$i]['object']=='JoomLeague15')
				{
					...
					JError::raiseNotice(500,JText::_('JL_ADMIN_XML_IMPORT_RENDERING_15'));
				}
...
//die();
		...
		{
			JError::raiseWarning(500,JText::sprintf('JL_ADMIN_XML_IMPORT_ERROR', 'Something is wrong inside the import file'));
			return false;
		}
	}
...
$this->dump_variable("this->_newplaygroundid", $this->_newplaygroundid);
$this->dump_variable("this->_dbplaygroundsid", $this->_dbplaygroundsid);
}
		...
		if (!empty($this->_dbplaygroundsid))
		{
			foreach ($this->_dbplaygroundsid AS $key => $id)
			{
				...
				$my_text .= '<span style="color:'.$this->existingInDbColor.'">';
				$my_text .= JText::sprintf(	'1.) Using existing playground data: %1$s',
		...
		if (!empty($this->_newplaygroundid))
		{
			foreach ($this->_newplaygroundid AS $key => $id)
			{
				...
				if ($object=$this->_db->loadObject())
				{
					...
					$my_text .= '<span style="color:'.$this->existingInDbColor.'">';
					$my_text .= JText::sprintf('2.) Using existing playground data: %1$s',"</span><strong>$object->name</strong>");
					$my_text .= '<br />';
				}
				else
				{
					...
					{
						$my_text .= '<span style="color:'.$this->storeFailedColor.'"><strong>';
						$my_text .= JText::_('3.) Error in function _importPlayground').'</strong></span><br />';
					...
					else
					{
						...
						$my_text .= '<span style="color:'.$this->storeSuccessColor.'">';
						$my_text .= JText::sprintf('4.) Created new playground data: %1$s',"</span><strong>$p_playground->name</strong>");
						$my_text .= '<br />';
					}
				}
			}
		...
		if (!empty($this->_dbclubsid))
		{
			...
			foreach ($this->_dbclubsid AS $key => $id)
			{
				if (empty($this->_newclubs[$key]))
				{
					...
					$my_text .= '<span style="color:'.$this->existingInDbColor.'">';
					$my_text .= JText::sprintf(	'1.) Using existing club data: %1$s - %2$s',
					...
					else
					{
						...
						$my_text .= '<span style="color:'.$this->storeSuccessColor.'">';
						$my_text .= JText::sprintf(	'1.b) Updated club data: %1$s - %2$s',
...
//$this->dump_variable("this->_newclubs", $this->_newclubs);
				...
				if ($object=$this->_db->loadObject())
				{
					...
					$my_text .= '<span style="color:'.$this->existingInDbColor.'">';
					$my_text .= JText::sprintf('2.) Using existing club data: %1$s',"</span><strong>$object->name</strong>");
					$my_text .= '<br />';
				}
				else
				{
					...
					{
						$my_text .= '<span style="color:'.$this->storeFailedColor.'"><strong>';
						$my_text .= JText::_('3.) Error in function importClubs').'</strong></span><br />';
					...
					else
					{
						...
						$this->_convertClubID[$oldID]=$insertID;
						$my_text .= '<span style="color:'.$this->storeSuccessColor.'">';
						$my_text .= JText::sprintf(	'4.) Created new club data: %1$s - %2$s',
													"</span><strong>$p_club->name</strong>",
													"".$p_club->country.""
													);
						$my_text .= '<br />';
...
//$this->dump_variable("p_matchstatistic", $p_matchstatistic);
	...
	public function importData($post)
	{
		...
		if (is_array($post) && count($post) > 0)
		{
          ...
          if ( $this->debug_info )
          {
          $this->dump_header("function importData");
          $this->dump_variable("post", $post);  
