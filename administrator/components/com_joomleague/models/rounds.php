	...
	*/
	...
	{		
		...
		if (!$teams || !count($teams)) {
			$this->setError(JText::_('JL_ADMIN_ROUNDS_POPULATE_ERROR_NO_TEAM'));
			return false;
		}
		$rounds = $this->getData();
		...
		{
			$this->setError(JText::_('JL_ADMIN_ROUNDS_POPULATE_ERROR_UNDEFINED_SCHEDULING'));
			return false;			
		}
		...
		foreach ($schedule as $k => $games)
		{
			...
			else // create the round !
			{
				...
				if (!($round->check() && $round->store())) {
					$this->setError(JText::_('JL_ADMIN_ROUNDS_POPULATE_ERROR_CREATING_ROUND').': '.$round->getError());
					return false;	
				}				
			...
			foreach ($games as $g)
			{
				...
				if (!($game->check() && $game->store())) {
					$this->setError(JText::_('JL_ADMIN_ROUNDS_POPULATE_ERROR_CREATING_GAME').': '.$game->getError());
					return false;	
				}
			}
		}
