...
JToolBarHelper::title( JText::_( 'XGALLERY COLLECTIONS' ), 'generic.png' );
JToolBarHelper::publishList();
JToolBarHelper::unpublishList();
JToolBarHelper::preferences('com_xgallery', 425);
JToolBarHelper::editList();
JToolBarHelper::deleteList(JText::_('CONFIRM DELETE COLLECTION'));
...
<table class="adminlist">
  <thead>
    <tr class="sortable">
	  ...
	  <th style="width:15px; text-align:center;"><?php echo JHTML::_( 'grid.sort', 'Category', 'cid', $this->lists['order_Dir'], $this->lists['order']); ?></th>
  ...
  for ($i = 0, $n = $m; $i < $n; $i++) {
	...
	$link = JFilterOutput::ampReplace( 'index.php?option=' . $option . '&controller=collections&view=collection&task=edit&id='. $row->id );
    ?>
    <tr class="<?php echo "row$k"; ?>">
      ...
      <td style="width:100px; text-align:center;">
        <?php if(isset($this->categories[$row->cid])) { 
        	echo $this->categories[$row->cid]; 
        } else {
        	echo JText::_('CATEGORY NOT FOUND');
        }
        ?>
      </td>
      <td style="width:15px; text-align:center;">
        <img src="<?php echo JURI::root(true).'/components/com_xgallery/helpers/img.php?file='.'/'.$row->thumb.'&w='.$this->rWidth.'&h='.$this->rHeight."&amp;tn=0"; ?>" style="border:none; height:75px;" />
      </td>
      <td style="width:15px; text-align:center;">
...
<input type="hidden" name="controller" value="collections" />
