 ...
 * @since 1.5
 */
class XGalleryControllerFile extends JController {
	...
	function delete()
	{
		...
		if ($tmpl == 'component') {
			...
			$mainframe->redirect('index.php?option=com_xgallery&task=add&controller=folders&view=mediaList&folder='.$folder.'&tmpl=component&currCont='.CURR_CONT.'&id='.CURR_ID.'&reqType='.CURR_TYPE);
		} else {
			$mainframe->redirect('index.php?option=com_xgallery&task=add&controller=folders&folder='.$folder);
		}
	}
}
