...
var MediaManager = {
    ...
    initialize: function(surl)
    {
        ...
        this.tree = new MooTreeControl({ div: 'media-tree_tree', mode: 'folders', grid: true, theme: 'components/com_xgallery/assets/mootree.gif', onClick:
                function(node){
                    target = $chk(node.data.target) ? node.data.target : '_self';
                    window.frames[target].location.href = node.data.url;
                }
            },{ text: 'Media', open: true, data: { url: surl, target: 'folderframe'}}); 
    ...
    onloadframe: function()
    {
        ...
        if (folder) {
            ...
            this.folderpath.value = basepath + lineSep + folder;
    ...
    setViewType: function(type, cont, id, rtype)
    {
        ...
        this._setFrameUrl('index.php?option=com_xgallery&'+cont+id+rtype+'task=add&controller=folders&view=mediaList&tmpl=component&folder='+folder+'&layout='+type);
    },
