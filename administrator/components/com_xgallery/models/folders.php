 ...
 * @since 1.5
 */
class XGalleryModelFolders extends JModel
{
	...
	function getList() {
		...
		if (strlen($current) > 0) {
			$basePath = $bpath.DS.$current;
		} else {
			$basePath = $bpath;
		}
		$mediaBase = str_replace(DS, '/', $bpath.'/');
			...
			foreach ($fileList as $file)
			{
				if (is_file($basePath.DS.$file) && substr($file, 0, 1) != '.' && strtolower($file) !== 'index.html') {
					...
					switch ($ext)
					{
						...
						// Non-image document
						default:
							$iconfile_32 = JPATH_ADMINISTRATOR.DS."components".DS."com_xgallery".DS."images".DS."mime-icon-32".DS.$ext.".png";
							if (file_exists($iconfile_32)) {
								$tmp->icon_32 = JURI::base(true)."/components/com_xgallery/images/mime-icon-32/".$ext.".png";
							} else {
								$tmp->icon_32 = JURI::base(true)."/components/com_xgallery/images/con_info.png";
							}
							$iconfile_16 = JPATH_ADMINISTRATOR.DS."components".DS."com_xgallery".DS."images".DS."mime-icon-16".DS.$ext.".png";
							if (file_exists($iconfile_16)) {
								$tmp->icon_16 = JURI::base(true)."/components/com_xgallery/images/mime-icon-16/".$ext.".png";
							} else {
								$tmp->icon_16 = JURI::base(true)."/components/com_xgallery/images/con_info.png";
							}
							$docs[] = $tmp;
							break;
					}
