...
class JoomleagueModelEditEvents extends JoomleagueModelProject
{
	...
	function _makePlayerOptions($teamid,$multiple=false,$already_sel=false)
	{
		...
		$playeroptions=array();
		if (! $multiple){ $playeroptions[0]=JText::_('JL_PLAYER_SELECT');}
		$config = $this->getTemplateConfig( "player" );
		foreach($teamplayers as $player)
		{
			$nameStr = JoomleagueHelper::formatName(null, $player->firstname, $player->nickname, $player->lastname, $config["name_format"]);
		...
		{
			$playeroptions[0]=JText::_('JL_PLAYER_SELECT');
		}
		return $playeroptions;
	}
	...
	function isAllowed()
	{
		...
		if ($user->id != 0)
		{
			...
			if(($user->authorize('com_joomleague','result_events_edit')) ||
	...
	function setAllowed()
	{
		...
		$results->config = $this->getTemplateConfig('results');
		$level = (int)$results->config['minimum_permission'];
		$user_level = array("super administrator","administrator","manager","publisher","editor","author","registered");

		for ($i=0;$i<=$level;$i++){
			$auth->addACL('com_joomleague','result_events_edit','users',$user_level[$i]);
		}
