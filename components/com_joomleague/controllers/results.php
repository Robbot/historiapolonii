...
class JoomleagueControllerResults extends JoomleagueController
{
	...
	function edit()
	{
		require_once( JPATH_COMPONENT . DS . 'js' . DS . 'JL_frontediting.js.php' );
	...
	function saveshort()
	{
		JRequest::checkToken() or jexit('JL_GLOBAL_INVALID_TOKEN');
		...
		if ((!$allowed) && (!$isMatchAdmin))
		{
			...
			$msg = JText::_('JL_SAVE_RESULTS_NO_PRIVILEGES');
		...
		for ($x = 0; $x < count($cid); $x++)
		{
			...
			{
				$msg = JText::_('JL_SAVE_RESULTS_SUCCESS');
			}
			else
			{
				$msg = JText::_('JL_SAVE_RESULTS_ERROR');
			}
		}
		$link = JoomleagueHelperRoute::getResultsRoute($project_id, $round_id);
		echo '#'.$msg.'#';
