...
<table width="100%" align="center" border="0" cellpadding="3"
	...
	<tr class="sectiontableheader">
		<th class="rank"><?php	echo JText::_( 'JL_STATSRANKING_RANK' );	?></th>

		<?php if ($this->config['show_picture_thumb'] == 1 ):	?>
		<th class="td_c">&nbsp;</th>
		<?php endif; ?>

		<th class="td_l"><?php	echo JText::_( 'JL_STATSRANKING_PLAYER_NAME' ); ?>
		</th>
		...
		<th class="td_l"><?php	echo JText::_( 'JL_STATSRANKING_TEAM' );	?></th>
		<?php endif; ?>
		<?php	if ( $show_icons == 1 ):	?>
	...
	<tr class="<?php echo $class; ?>"<?php echo $favStyle; ?>>
		...
		<td class="td_l playerteam">
			...
			if ($isFavTeam) {
				...
				$this->config['highlight_fav'] = $isFavTeam;
    		$this->config['show_icons'] = 0;
				$this->config['names'] = $row->team_name;
				$teamName = JoomleagueHelper::formatTeamName($team,"t".$row->team_id,$this->config, $isFavTeam,0,$this->project->id);
			} else {
			  $teamName = $row->team_name;
			}

			if ( ( $this->config['link_to_team'] > 0 ) &&
				( $this->project->id > 0 ) && ( $row->team_id > 0 ) )
			{
				if ( $this->config['link_to_team'] == 1 ) {
					$link = JoomleagueHelperRoute::getTeamInfoRoute( $this->project->id, $row->team_id  );
				}
				elseif ( $this->config['link_to_team'] == 2 ) {
					$link = JoomleagueHelperRoute::getPlayersRoute( $this->project->id, $row->team_id  );
				}
				echo JHTML::link( $link, $teamName );
			}
			else
			{
				echo $teamName;
...
<?php echo JHTML::link(JoomleagueHelperRoute::getStatsRankingRoute($this->project->id, ($this->division ? $this->division->id : 0), $this->teamid, $rows->id), JText::_('JL_STATSRANKING_VIEW_FULL_TABLE')); ?>
</div>
<?php
}
else
