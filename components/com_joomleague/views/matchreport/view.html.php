...
class JoomleagueViewMatchReport extends JLGView
{
	...
	function display($tpl=null)
	{
		...
		$this->assignRef('matchevents',$model->getMatchEvents($this->match->id,1,($this->config['sort_events_desc'])));
		...
		$paramsdata=$match->extended;
		$paramsdefs=JLG_PATH_ADMIN.DS.'assets'.DS.'extended'.DS.'match.xml';
		$extended=new JLGExtraParams($paramsdata,$paramsdefs);
		$this->assignRef('extended',$extended);

		// Set page title
		$pageTitle = JText::_( 'JL_MATCHREPORT_PAGE_TITLE' );
		if (( isset( $this->team1 ) ) AND (isset( $this->team1 )))
		{
			$pageTitle .= ": ".$this->team1->name." ".JText::_( "JL_NEXTMATCH_VS" )." ".$this->team2->name;
		}
	...
	{
		$pic_time='media/com_joomleague/event_icons/playtime.gif';
		$pic_out='media/com_joomleague/event_icons/out.png';
		$pic_in='media/com_joomleague/event_icons/in.png';

		//$imgTitle=JText::_('JL_MATCHREPORT_MINUTE');
		//$imgTitle2=array(' title' => $imgTitle);
		//$result=JHTML::image($pic_time,$imgTitle,$imgTitle2).'&nbsp;'.$sub->in_out_time;
		$result='<b>'.$sub->in_out_time.'. '. JText::_('JL_MATCHREPORT_MINUTE') .'</b>';
		$result .= '<br />';
		$outName = JoomleagueHelper::formatName(null, $sub->out_firstname, $sub->out_nickname, $sub->out_lastname, $this->config["name_format"]);
		if($outName != '') {
			$imgTitle=JText::_('JL_MATCHREPORT_SUBSTITUTION_WENT_OUT');
			$imgTitle2=array(' title' => $imgTitle);
		...
		if($inName!='') {
			$imgTitle=JText::_('JL_MATCHREPORT_SUBSTITUTION_CAME_IN');
			$imgTitle2=array(' title' => $imgTitle);
	...
	function _formatTimelineSubstitution($sub,$firstname,$nickname,$lastname,$out_firstname,$out_nickname,$out_lastname,$two_substitutions_per_minute=0)
	{
		...
		$pic_out='media/com_joomleague/event_icons/out.png';
		$pic_in='media/com_joomleague/event_icons/in.png';
		$pic_time='media/com_joomleague/event_icons/change.png';

		$time=$sub->in_out_time;
                $matchtime=$this->getTimelineMatchTime();
                $time2=($time / $matchtime) *100;
		$tiptext=JText::_('JL_MATCHREPORT_TIMELINE_SUBSTITUTION_MIN').' ';
		$tiptext .= $time;
		$tiptext .= ' ::';
	...
	function _formatTimelineEvent($matchEvent,$event,$firstname,$nickname,$lastname,$picture,$two_events_per_minute=0)
	{
		...
		$tiptext=JText::_($event->name).' '.JText::_('JL_MATCHREPORT_MINUTE_SHORT').' '.$matchEvent->event_time;
		$tiptext .= ' ::';
