...
class JoomleagueViewTeamPlan extends JLGView
{
	function display($tpl=null)
	{
		...
		$document->addScript(JPATH_SITE.DS.'includes/js/overlib_mini.js');
		...
		if (isset($project))
		{
			...
			$this->assignRef('teamsrefplan',$model->getTeamsRefPlan($config));
			$this->assignRef('matchesperround',$model->getMatchesPerRound($config,$rounds));
			$this->assignRef('model',$model);

			//if ($model->getMode() > 0){$this->setLayout('results');}
		}
		...
		$document->setTitle(JText::sprintf('JL_TEAMPLAN_PAGE_TITLE',$pageTitle));
