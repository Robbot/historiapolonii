...
else
{
	...
	?>
		...
		{
			$club_emblem_title = str_replace( "%CLUBNAME%", $this->club->name, JText::_( 'JL_CLUBINFO_EMBLEM_TITLE' ) );
		...
		{
			$club_trikot_title = str_replace( "%CLUBNAME%", $this->club->name, JText::_( "JL_CLUBINFO_TRIKOT_TITLE" ) );
	...
	<div class="right-column">
		...
		if ( ( $this->club->address ) || ( $this->club->zipcode ) )
		{
			...
			$addressString = Countries::convertAddressString(	$this->club->name,
																...
																$this->club->location,
																$this->club->country,
																'JL_CLUBINFO_ADDRESS_FORM' );
			?>
			<span class="clubinfo_listing_item"><?php
				echo JText::_( 'JL_CLUBINFO_ADDRESS' );
		...
		{
			?>
			<span class="clubinfo_listing_item"><?php echo JText::_( 'JL_CLUBINFO_PHONE' ); ?></span>
		...
		{
			?>
			<span class="clubinfo_listing_item"><?php echo JText::_( 'JL_CLUBINFO_FAX' ); ?></span>
		...
		{
			?>
			<span class="clubinfo_listing_item"><?php echo JText::_( 'JL_CLUBINFO_EMAIL' ); ?></span>
		...
		{
			?>
			<span class="clubinfo_listing_item"><?php echo JText::_( 'JL_CLUBINFO_WWW' ); ?></span>
			<span class="clubinfo_listing_value">
		...
		{
			?>
			<span class="clubinfo_listing_item"><?php echo JText::_( 'JL_CLUBINFO_PRESIDENT' ); ?></span>
		...
		{
			?>
			<span class="clubinfo_listing_item"><?php echo JText::_( 'JL_CLUBINFO_MANAGER' ); ?></span>
		...
		{
			?>
			<span class="clubinfo_listing_item"><?php echo JText::_( 'JL_CLUBINFO_FOUNDED' ); ?></span>
		...
		if ( ( $this->config['show_playgrounds_of_club'] == 1 ) && ( isset( $this->stadiums ) ) && ( count( $this->stadiums ) > 0 ) )
		{
			...
			<?php
				...
				foreach ( $this->playgrounds AS $playground )
				{
					...
					$pl_dummy = JText::_( 'JL_CLUBINFO_PLAYGROUND' );
					?>
