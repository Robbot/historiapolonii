...
class JoomleagueViewResultsranking extends JoomleagueViewResults {
	...
	function display($tpl = null)
	{
		...
		$options = $this->getRoundSelectNavigation($rounds, $resultsmodel->divisionid);
		
		$this->assignRef('matchdaysoptions',$options);
		$this->assignRef('currenturl', 		JoomleagueHelperRoute::getResultsRankingRoute($resultsmodel->getProject()->slug, $this->round, $resultsmodel->divisionid));
	...
	function getRoundSelectNavigation(&$rounds, $division=0)
	{
		$options = array();
		foreach ($rounds as $r)
		{
			$link = JoomleagueHelperRoute::getResultsRankingRoute($this->project->slug, $r->value, $division);
