...
<table class='fixtures-results'>
	...
	<tr class="sectiontableheader">
		...
		{
			?>
			<th colspan="<?php echo $nbcols-$nbcols_header; ?>"><?php echo JHTML::date( $date, JText::_( 'JL_RESULTS_GAMES_DATE' ) );
                if ($this->config['show_matchday_dateheader']) {
                    echo ' - ' . JText::sprintf( 'JL_RESULTS_GAMEDAY_NB',$this->roundcode ); } ?>
            </th>
            <?php
            if ($this->config['show_attendance_column']) {
				?>
				<th class="right"><?php echo JText::_( 'JL_RESULTS_ATTENDANCE' ); ?></th>
			<?php
			}
            if ($this->config['show_comments_count'] > 0) {
				?>
				<th class="center"><?php echo JText::_( 'JL_RESULTS_COMMENTS' ); ?></th>
			<?php
			}

		} else {
			?>
			<th colspan="<?php echo $nbcols; ?>"><?php echo JHTML::date( $date, JText::_( 'JL_RESULTS_GAMES_DATE' ) );
                if ($this->config['show_matchday_dateheader']) {
                    echo ' - ' . JText::sprintf( 'JL_RESULTS_GAMEDAY_NB',$this->roundcode ); } ?>
            </th>
		<?php
...
//--------------------------------------------------------------------------------------------------------------
	?>
		...
		<td class="center"><?php
			...
			if ($count == 1) {
				$imgTitle		= $count.' '.JText::_('JL_RESULTS_COMMENTS_COUNT_SINGULAR');
			...
			elseif ($count > 1) {
				$imgTitle	= $count.' '.JText::_('JL_RESULTS_COMMENTS_COUNT_PLURAL');
				if ($this->config['show_comments_count'] == 1) {
					$href_text		= JHTML::image( JURI::root().'media/com_joomleague/jl_images/discuss_active.gif', $imgTitle, array(' title' => $imgTitle,' border' => 0,' style' => 'vertical-align: middle'));
				} elseif ($this->config['show_comments_count'] == 2) {
					$href_text		= '<span title="'. $imgTitle .'">('.$count.')</span>';
				...
				$imgTitle	= JText::_('JL_RESULTS_COMMENTS_COUNT_NOCOMMENT');
				if ($this->config['show_comments_count'] == 1) {
