...
class JoomleagueViewResults extends JLGView
{
	...
	public function display($tpl = null)
	{
		...
		// Set page title
		$pageTitle = JText::_('JL_RESULTS_PAGE_TITLE');
	...
	public function showNotPlayingTeams(&$games,&$teams,&$config,&$favteams,&$project)
	{
		...
		if ($not_playing > 0)
		{
			$output .= '<b>'.JText::sprintf('JL_RESULTS_TEAMS_NOT_PLAYING',$not_playing).'</b> ';
	...
	public function editPartResults($i,$match,$backend=false)
	{
		...
		$imgTitle=JText::_('JL_ADMIN_EDIT_MATRIX_ROUNDS_PART_RESULT');
		...
		if ($this->project->allow_add_time)
		{
			if($match->match_result_type >0){
				echo JText::_('JL_RESULTS_OVERTIME').':';
				echo '<input type="text" style="font-size:9px;"';
			...
			if($match->match_result_type == 2){
				echo '<br />';
				echo JText::_('JL_RESULTS_SHOOTOUT').':';
				echo '<input type="text" style="font-size:9px;"';
	...
	*/
	...
	function showMatchRefereesAsTooltip(&$game)
	{
		if ($this->config['show_referee'])
		{
			...
			if (!empty($referees))
			{
				$toolTipTitle	= JText::_('JL_RESULTS_REF_TOOLTIP');
				$toolTipText	= '';
	...
	function showEventsContainerInResults($matchInfo,$projectevents,$matchevents,$substitutions=null,$config)
	{
		...
		if ($this->config['use_tabs_events'])
		{
			...
			if (!empty($substitutions))
			{
				...
				{
					// Event icon as thumbnail on the tab (a placeholder icon is used when the icon does not exist)
					$imgTitle = JText::_('JL_IN_OUT');
					$pic_tab	= 'media/com_joomleague/event_icons/subst.png';
					$tab_content = JoomleagueHelper::getPictureThumb($pic_tab, $imgTitle, $width, $height, $type);
				}
				else
				{
					$tab_content = JText::_('JL_IN_OUT');
				}

				$pic_time	= JURI::root().'media/com_joomleague/event_icons/playtime.gif';
				$pic_out	= JURI::root().'media/com_joomleague/event_icons/out.png';
				$pic_in		= JURI::root().'media/com_joomleague/event_icons/in.png';
				$imgTime = JHTML::image($pic_time,JText::_('JL_MATCHREPORT_SUBSTITUTION_MINUTE'),array(' title' => JText::_('JL_MATCHREPORT_SUBSTITUTION_MINUTE')));
				$imgOut  = JHTML::image($pic_out,JText::_('JL_MATCHREPORT_SUBSTITUTION_WENT_OUT'),array(' title' => JText::_('JL_MATCHREPORT_SUBSTITUTION_WENT_OUT')));
				$imgIn   = JHTML::image($pic_in,JText::_('JL_MATCHREPORT_SUBSTITUTION_CAME_IN'),array(' title' => JText::_('JL_MATCHREPORT_SUBSTITUTION_CAME_IN')));
	...
	function formatResult(&$team1,&$team2,&$game,&$reportLink)
	{
		...
		if($this->config['show_link_matchreport'] == 1 || ($this->config['show_link_matchreport'] == 2 && $fav))
		{
			...
			array("title" => JText::_('JL_RESULTS_SHOW_MATCHREPORT')));
		}
		else
		{
	...
	function _formatEventContainerInResults($matchevent, $event, $projectteamId, $showEventInfo)
	{
		...
		if ($matchevent->event_type_id == $event->id && $matchevent->ptid == $projectteamId)
		{
			...
			{
				$output .= JText :: _('JL_GLOBAL_UNKNOWN_PERSON');
			}
	...
	function _formatSubstitutionContainerInResults($subs,$projectteamId,$imgTime,$imgOut,$imgIn)
	{
		...
		if ($subs->ptid == $projectteamId)
		{
			$output .= '<li class="events">';
			// $output .= $imgTime;
			$output .= '&nbsp;'.$subs->in_out_time.'. '.JText::_('JL_MATCHREPORT_SUBSTITUTION_MINUTE');
			$output .= '<br />';
