...
class JLGRanking
{
	 ...
	 * @param string $type
	 */
	function &getInstance($project = null)
	{
		if ($project)
		{
	...
	{
		if (!$this->_projectid) {
			JError::raiseWarning(0, JText::_('JL_RANKING_ERROR_PROJECTID_REQUIRED'));
			return false;
		}
...
// echo '</pre>';
	...
	function _getRoundcode($round_id)
	{
		...
		if (!isset($this->_roundcodes[$round_id])) {
			JError::raiseWarning(0, JText::_('JL_RANKING_ERROR_UNKOWN_ROUND_ID').': '.$round_id);
			return false;
		}
